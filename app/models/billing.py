...
class Subscription(Base):
    ...
    plan_type: Mapped[SubPlan] = mapped_column(Enum(SubPlan,name='subplan'), nullable=False)
    ...
    status: Mapped[SubStatus] = mapped_column(Enum(SubStatus,name='substatus'), nullable=False, default=SubStatus.active, index=True)
...
class AttendanceBilling(Base):
    ...
    billing_status: Mapped[BillingStatus] = mapped_column(Enum(BillingStatus,name='billingstatus'), nullable=False, default=BillingStatus.pending, index=True)
...
class Invoice(Base):
    ...
    status: Mapped[InvoiceStatus] = mapped_column(Enum(InvoiceStatus,name='invoicestatus'), nullable=False, default=InvoiceStatus.draft, index=True)
...
class RevenueSplit(Base):
    ...
    payout_status: Mapped[PayoutStatus] = mapped_column(Enum(PayoutStatus,name='payoutstatus'), nullable=False, default=PayoutStatus.pending, index=True)
...
class Payment(Base):
    ...
    method: Mapped[PaymentMethod] = mapped_column(Enum(PaymentMethod,name='paymentmethod'), nullable=False)
    provider_ref: Mapped[str | None] = mapped_column(String(100), unique=True)
    status: Mapped[PaymentStatus] = mapped_column(Enum(PaymentStatus,name='paymentstatus'), nullable=False, default=PaymentStatus.pending, index=True)
